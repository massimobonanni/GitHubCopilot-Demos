...
| [4](#demo-4) | `04-documentation` | Developer Flow → Document | **JSDoc & README generation** | 5 min |
...
4. Ask Chat:
   ...
   Convert this to ES modules with arrow functions
...
1. Open `weatherClient.js` → Chat →

   ```text

   Add JSDoc comments to every method

   ```

2. Select `getTemperature()` → Inline Chat →

   ```text

   Add a detailed JSDoc with @param, @returns, @throws
...
4. Ask Chat:
   ...
   Convert this to TypeScript with full type annotations
...
4. Ask Chat:
   ...
   Fix it using a Set for O(n) duplicate detection
...
3. Type:
   ...
   Add SQLite persistence using better-sqlite3 so tasks survive a restart. Keep the CLI.
...
| File | Purpose |
| ------ | --------- |
| `notificationService.js` | Demo target — undocumented JS notification service (Email / SMS / Push) |
...
1. Open `notificationService.js`, switch to **Agent** mode
2. Ask:

   ```text

   Add JSDoc comments to all public members
...
5. **Trigger the deny:** Ask the agent *"Delete all compiled files using rm -rf dist/"* — watch `pre-tool-guard` block it
...
- `shoppingCart.js` — a shopping cart with an intentional discount bug
...
2. **Create the Issue** — either paste `ISSUE.md` into a new issue in the GitHub UI,
   ...
   gh issue create --title "Percentage coupons applied as a flat amount" \
     --body-file Javascript/14-issue-to-pr/ISSUE.md
...
3. **Let Copilot fix it** — choose one path:
   ...
   - **Copilot Chat (interactive):** in VS Code switch to **Agent** mode and ask:
     ...
     *"Fix the bug described in `14-issue-to-pr/ISSUE.md` (issue #N): make `applyDiscount`
     honor `coupon.isPercentage`, and add Jest tests for both coupon types."*

4. **Review the Pull Request** — open the PR, read the diff, and run the tests with `npm test`.
...
- `textAnalyzer.js` — a small text-analysis class with only word/character counting implemented
...
`estimateReadingTimeMinutes`, `topWords`), add Jest tests, refresh the demo output,
...
4. **Watch the session** — open the live agent session and narrate what it does: reads the repo,
   ...
   plans the change, edits `textAnalyzer.js`, adds tests, and runs the build/tests. You can keep
...
- `promoService.js` — the feature implementation under review (promo-code redemption at checkout, with two intentional defects)
- `promoService.test.js` — the Jest tests submitted with the PR (they all **pass**, but only cover the happy paths)
...
**The scenario:** the PR adds `PromoService.apply(code, subtotal)`. It runs and the three
submitted tests pass — but it hides **two defects** the quality gate should catch by
...
1. Copy all three agent files into the workspace agents folder:
   ...
   cp Javascript/16-multi-agent-pr-review/pr-quality-gate.agent.md      .github/agents/
   cp Javascript/16-multi-agent-pr-review/requirements-analyst.agent.md .github/agents/
   cp Javascript/16-multi-agent-pr-review/implementation-reviewer.agent.md .github/agents/
...
1. Put this folder's changes on a feature branch and push it:
   ...
   git add Javascript/16-multi-agent-pr-review/promoService.js Javascript/16-multi-agent-pr-review/promoService.test.js
...
2. Open the PR, using the contents of `PR.md` as the description (paste it, or use the CLI):
   ...
   gh pr create --title "Add promo-code redemption at checkout" \
     --body-file Javascript/16-multi-agent-pr-review/PR.md
...
2. Switch Chat to the **pr-quality-gate** agent and start the review:
   ...
   Javascript/16-multi-agent-pr-review/PR.md and the changed files are promoService.js
   and promoService.test.js.
...
3. **Narrate the delegation** as it happens:
   ...
   - it then calls **`implementation-reviewer`** → reads the diff, runs `npx jest` (all pass), and maps each AC to evidence
   - the coordinator **reconciles** both handoffs into one report

4. **Read the decision** — expected result is **BLOCK**, with evidence:
   - AC-04 → *Not met* (`promoService.js` — case-sensitive `p.code === code`)
   - AC-05 → *Partial* (`promoService.js` — `subtotal > promo.minimumSubtotal` should be `>=`)
   - AC-07 → *Partial* (tests pass but miss the case-insensitive and boundary cases)
   - Finding → opaque `TypeError` when `find()` returns undefined for an unknown code
...
defines ticket ordering and owner-workload rules, while Node's built-in test runner
provides a failing baseline and executable acceptance criteria.

**How to demo:**

1. Open a terminal in `17-github-copilot-cli` and run `copilot`.
2. Use `@TASK.md` and `@issueDesk.js` to ask for an explanation.
3. Press **Shift+Tab**, ask Copilot to plan the implementation, and review the plan.
4. Return to normal mode and ask Copilot to implement the plan and run `npm test`.
5. Discuss each file/command approval instead of granting broad permissions.
6. Run `!git diff -- issueDesk.js issueDesk.test.js`, then ask Copilot to review it.
...
center for planning, running, reviewing, and validating multiple JavaScript
...
overdue and due-soon items. Separate source and Node.js test files let the
workstreams be delegated in parallel and reviewed independently.
