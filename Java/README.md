...
4. Ask Chat:
   ...
   Add JavaDoc comments to all public methods in this file
...
1. Open `WeatherClient.java` → Chat →

   ```text

   Add comprehensive JavaDoc to every public method
...
2. Select `getTemperature()` → Inline Chat →
   ...
   Add detailed JavaDoc with @param, @return, @throws
...
3. Ask Chat:
   ...
   Write a README.md explaining how to use this module with code examples
...
4. Ask Chat:
   ...
   Are there any null pointer risks I should document?
...
3. Select `hashPassword()` → Inline Chat →
   ...
   Is this secure? What should I use instead?
...
4. Ask Chat:
   ...
   Rewrite this module following OWASP authentication best practices
...
2. Ask:
   ...
   Add comprehensive JavaDoc to all public members
...
defines ticket ordering and owner-workload rules, while the JUnit suite provides
a failing baseline and executable acceptance criteria.

**How to demo:**

1. Open a terminal in `17-github-copilot-cli` and run `copilot`.
2. Use `@TASK.md` and `@src/main/java/com/githubcopilot/demo/TicketQueue.java` to ask for an explanation.
3. Press **Shift+Tab**, ask Copilot to plan the implementation, and review the plan.
4. Return to normal mode and ask Copilot to implement the plan and run `mvn test`.
5. Discuss each file/command approval instead of granting broad permissions.
6. Run `!git diff -- src/main src/test`, then ask Copilot to review it.
...
center for planning, running, reviewing, and validating multiple Java
...
overdue and due-soon items. Separate source and JUnit test files let the
