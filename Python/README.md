...
| [4](#demo-4) | `04-documentation` | Developer Flow → Document | **Docstring & README generation** | 5 min |
...
1. Open `weather_client.py` → Chat →

   ```text

   Add Google-style docstrings to every method

   ```

2. Select `get_temperature()` → Inline Chat →

   ```text

   Add a detailed docstring with Args, Returns, Raises
...
3. Select `hash_password()` → Inline Chat →
...
3. Type:
   ...
   Add SQLite persistence so tasks survive a restart. Keep the CLI.
...
5. Ask:
   ...
   Rewrite `fetch_user_orders` to fix the issues you found
...
2. Ask:
   ...
   Add Google-style docstrings to all public members
...
- `promo_service.py` — the feature implementation under review (promo-code redemption at checkout, with two intentional defects)
- `test_promo_service.py` — the pytest tests submitted with the PR (they all **pass**, but only cover the happy paths)
...
- **AC-05 partial** — a `>` instead of `>=` wrongly rejects a promo at exactly its `minimum_subtotal` (off-by-one).
...
1. Copy all three agent files into the workspace agents folder:
   ...
   cp Python/16-multi-agent-pr-review/pr-quality-gate.agent.md      .github/agents/
   cp Python/16-multi-agent-pr-review/requirements-analyst.agent.md .github/agents/
   cp Python/16-multi-agent-pr-review/implementation-reviewer.agent.md .github/agents/
...
1. Put this folder's changes on a feature branch and push it:
   ...
   git add Python/16-multi-agent-pr-review/promo_service.py Python/16-multi-agent-pr-review/test_promo_service.py
...
2. Open the PR, using the contents of `PR.md` as the description (paste it, or use the CLI):
   ...
   gh pr create --title "Add promo-code redemption at checkout" \
     --body-file Python/16-multi-agent-pr-review/PR.md
...
2. Switch Chat to the **pr-quality-gate** agent and start the review:
   ...
   Python/16-multi-agent-pr-review/PR.md and the changed files are promo_service.py
   and test_promo_service.py.
...
3. **Narrate the delegation** as it happens:
   ...
   - it then calls **`implementation-reviewer`** → reads the diff, runs `pytest` (all pass), and maps each AC to evidence
   - the coordinator **reconciles** both handoffs into one report

4. **Read the decision** — expected result is **BLOCK**, with evidence:
   - AC-04 → *Not met* (`promo_service.py` — case-sensitive `p.code == code`)
   - AC-05 → *Partial* (`promo_service.py` — `subtotal > promo.minimum_subtotal` should be `>=`)
   - AC-07 → *Partial* (tests pass but miss the case-insensitive and boundary cases)
   - Finding → opaque `StopIteration` from `next()` on an unknown code
