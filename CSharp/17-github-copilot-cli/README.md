...
This starter demonstrates an end-to-end coding task from the terminal. The
`TicketQueue` compiles, but two methods intentionally throw
`NotImplementedException`. The tests and `TASK.md` define the expected behavior.
...
Before presenting, run this from the demo folder to restore packages and confirm
that the starter fails only because the two methods are unfinished:

```powershell
dotnet test .\tests\IssueDesk.Tests\IssueDesk.Tests.csproj
```
...
1. Start Copilot CLI in the demo folder:
   ...
   ```powershell
   cd CSharp\17-github-copilot-cli
   copilot
   ```
...
3. Demonstrate explicit file context without making changes:
   ...
   ```text
   Explain @TASK.md and @src/IssueDesk/TicketQueue.cs. Identify the unfinished
   behavior and the tests that should prove it. Do not edit files yet.
   ```
...
4. Press **Shift+Tab** to enter plan mode, then prompt:
   ...
   tests. Include the validation commands you will run.
   ```
...
5. Ask Copilot to complete the task:
   ...
   for uncovered edge cases, and run the focused test project when finished.
   ```
   ...
   file edits and `dotnet` commands needed for this task.

6. Run a shell command directly from the Copilot prompt:

   ```text
   !git diff -- src/IssueDesk tests/IssueDesk.Tests
   ```
...
extra tests Copilot created:

```powershell
git restore CSharp/17-github-copilot-cli
git clean -fd CSharp/17-github-copilot-cli
```
